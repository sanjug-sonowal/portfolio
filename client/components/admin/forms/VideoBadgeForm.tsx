...
import { TextInput, FileInput } from "@/components/common";
import { TYPOGRAPHY } from "@/constants/typography";
import type { Badge } from "@/components/badges/types";

export interface VideoBadgeFormProps {
  initialData?: Badge;
...
export function VideoBadgeForm({
  initialData,
  onSubmit,
  onCancel,
  isLoading = false,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!videoFile && !initialData?.file && !link.trim()) {
      newErrors.file = "Either video file or link is required";
    }
    ...
    if (Object.keys(newErrors).length === 0) {
      const badgeData: Badge = {
        id: initialData?.id || `video-badge-${Date.now()}`,
        title: title.trim(),
        type: "video",
        file: videoFile || initialData?.file || "",
